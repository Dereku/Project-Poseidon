...
public enum EnumArt {
    ...
    private static final HashMap<String, EnumArt> BY_NAME = new HashMap<>();
    private static final HashMap<Integer, EnumArt> BY_ID = new HashMap<>();

    static {
        for (EnumArt art : EnumArt.values()) {
            BY_NAME.put(art.A, art);
            BY_ID.put(art.id, art);
        }
    }

    public static EnumArt getByName(final String name) {
        return BY_NAME.get(name);
    }

    public static EnumArt getById(final int id) {
        return BY_ID.get(id);
    }
